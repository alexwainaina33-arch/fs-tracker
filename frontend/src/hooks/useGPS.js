...
// v2: Privacy consent gate — field staff must explicitly allow GPS tracking.
//     Adds pause/resume so staff can temporarily suspend tracking.
...
const CONSENT_KEY = "fieldtrack_gps_consent"; // localStorage key
...
export function useGPS() {
      ...
      try {
        const { inside, breaches } = checkGeofences(
          ...
          zonesRef.current.map((z) => ({
            id: z.id, center_lat: z.latitude, center_lng: z.longitude,
            radius_metres: z.radius_meters, alert_on_exit: z.alert_on_exit,
        ...
        for (const breach of breaches) {
          ...
          await pb.collection("ft_geofence_events").create({
            user: user.id, geofence: breach.zone.id, event_type: breach.type,
            ...
            .then((managers) => {
              for (const mgr of managers) {
                pb.collection("ft_notifications").create({
                  recipient: mgr.id, type: `geofence_${breach.type}`,
                  title: breach.type === "exit" ? "⚠️ Geofence Exit" : "📍 Geofence Entry",
                  body: msg, reference_type: "ft_geofence_events", is_read: false,
                }).catch(() => {});
              }
            }).catch(() => {});
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      (p) => {
        const pos = {
          ...
          accuracy: p.coords.accuracy, speed: p.coords.speed ?? 0,
        };
  ...
  // ── Pause / Resume (privacy: lets staff temporarily suspend without full stop) ──
  const pause = useCallback(() => {
    pausedRef.current = true;
    setPaused(true);
    toast("GPS paused — your location won't be shared until you resume.", { icon: "⏸", duration: 4000 });
  }, []);
  ...
  const revokeConsent = useCallback(() => {
    ...
    setConsent(false);
    stop();
    toast("GPS tracking disabled. You can re-enable it in your profile.", { duration: 5000 });
  }, [stop]);

  // Auto-start for field staff — only after consent granted
  useEffect(() => {
...
// One-shot — used by visit/clock forms

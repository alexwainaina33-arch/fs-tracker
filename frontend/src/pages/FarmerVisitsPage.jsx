...
// Structured farm visit capture — farmer details, crops, GPS, photos
...
import {
  Leaf, Plus, Camera, MapPin, Phone, User, Sprout,
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <Badge
              label={visit.visit_outcome?.replace(/_/g, " ")}
              color={OUTCOME_COLORS[visit.visit_outcome] ?? "default"}
              size="xs"
            />
          ...
          {visit.farm_name && (
            <p className="text-xs text-[#8b95a1] mt-0.5">📍 {visit.farm_name}</p>
          )}
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            ...
            {visit.acreage && (
              <span className="text-xs text-[#8b95a1]">
                🌾 {visit.acreage} {visit.acreage_unit}
              </span>
            )}
          </div>
          {crops.length > 0 && (
            <div className="flex gap-1 mt-2 flex-wrap">
              ...
              {crops.length > 3 && (
                <span className="text-[10px] text-[#8b95a1]">+{crops.length - 3}</span>
              )}
            </div>
          )}
        </div>
        <div className="text-right flex-shrink-0">
          ...
          {visit.expand?.staff && (
            <p className="text-[10px] text-[#8b95a1] mt-1">{visit.expand.staff.name}</p>
          )}
        </div>
      </div>
    </div>
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${
                isSelected
                  ...
                  : "bg-[#21272f] text-[#8b95a1] hover:text-white"
              }`}
            >
              {crop}
            </button>
...
export default function FarmerVisitsPage() {
  ...
  const handleCameraCapture = (photo) => {
    setCamOpen(false);
    if (capturedPhotos.length >= 4) return toast.error("Maximum 4 photos per visit");
    ...
    <div className="p-5 max-w-3xl mx-auto space-y-5 pb-8">
      ...
      <div className="space-y-3">
        ...
        {data?.items.map((visit) => (
          <VisitCard key={visit.id} visit={visit} onView={setSelected} />
        ))}
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Record Farm Visit" width="max-w-xl">
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input
              ...
              onChange={(e) => set("farmer_phone", e.target.value)}
            />
            <Input
              label="Farm / Plot Name"
              placeholder="e.g. Shamba ya Kilimani"
          ...
          {position && (
            <div className="flex items-center gap-2 px-3 py-2 bg-[#00c096]/10 border border-[#00c096]/20 rounded-xl">
              ...
              <span className="text-xs text-[#00c096]">
                Farm GPS captured (±{Math.round(position.accuracy)}m)
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Select
              label="Unit"
              value={form.acreage_unit}
              onChange={(e) => set("acreage_unit", e.target.value)}
            >
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <div
                ...
                className={`w-10 h-5 rounded-full transition-colors relative ${
                  form.irrigation ? "bg-[#c8f230]" : "bg-[#21272f]"
                }`}
              >
                <div className={`w-4 h-4 rounded-full bg-white absolute top-0.5 transition-transform ${
                  form.irrigation ? "translate-x-5" : "translate-x-0.5"
                }`} />
              </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            <Textarea
              label="Current Inputs Used"
              placeholder="What agro-inputs does the farmer currently use?"
              rows={2}
              value={form.current_inputs}
              onChange={(e) => set("current_inputs", e.target.value)}
            />
            <Textarea
              label="Products Recommended"
              placeholder="Products/solutions you recommended to the farmer"
              rows={2}
            ...
            <Textarea
              label="Products Sold"
              placeholder="Products actually sold / orders taken"
              rows={2}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Select
              ...
              onChange={(e) => set("visit_outcome", e.target.value)}
            >
          ...
          <Textarea
            label="Field Notes"
            placeholder="Observations, issues, opportunities…"
            rows={3}
            value={form.notes}
                ...
                {capturedPhotos.map((p, i) => (
                  <div key={i} className="relative w-16 h-16 rounded-xl overflow-hidden border border-[#21272f]">
                    ...
                    <button
                      ...
                      className="absolute top-0.5 right-0.5 w-4 h-4 rounded-full bg-[#ff4d4f] text-white flex items-center justify-center text-[10px]"
                    >×</button>
                  </div>
                ))}
              </div>
        ...
        <div className="flex gap-3 pt-5 border-t border-[#21272f] mt-5">
          ...
          <Btn
            ...
            className="flex-1"
          >
            {createMut.isPending ? "Saving…" : "Save Visit Record"}
          </Btn>
        </div>
      </Modal>
        ...
        <Modal open={!!selected} onClose={() => setSelected(null)} title="Farm Visit Details" width="max-w-md">
          <div className="space-y-3 text-sm max-h-[70vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {selected.sub_county && <div><p className="text-[#8b95a1] text-xs">Sub-county</p><p className="text-white">{selected.sub_county}</p></div>}
              ...
              {selected.soil_type && <div><p className="text-[#8b95a1] text-xs">Soil</p><p className="text-white capitalize">{selected.soil_type.replace("_"," ")}</p></div>}
            </div>
            ...
            <p className="text-[10px] text-[#4a5568] font-mono">{"�"}</p>
          </div>
